...
void CInnerInterface::ResetControl()
{
	...
	m_pItemShopWindow = NULL; 	//itemmall
	...
	m_fItemShopDelay = 5.0f; //itemmall
	pButton = NULL;
...
// #endif
	...
	{
		if ( !IsVisibleGroup ( PLAYER_RANKING_DISPLAY ) )
		{
			GLGaeaClient::GetInstance().ReqPlayerRankingInfo();
...
//dmk14 | 11-4-16 | pk ranking
void CInnerInterface::RefreshPlayerRanking()
{
	if ( m_pPlayerRankingDisplay && m_pPlayerRankingDisplay->IsVisible() )
		m_pPlayerRankingDisplay->RefreshSwPRanking();
}
