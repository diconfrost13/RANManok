...
{
	CString strTempMap = "Revenge Ran Online";

	if (!n_strMapName.IsEmpty())
	{
		strTempMap.Format ("<%s>",n_strMapName.GetString() );
	}
	...
	const D3DXVECTOR2 vmapnamebackTexPos ( 0, 80 );
	...
	D3DXVECTOR2 vmapnamebackAlignSize;
	vmapnamebackAlignSize.x = static_cast<float>(floor(vmapnamebackSize.x * fWidth / fRealImageX));
	vmapnamebackAlignSize.y = static_cast<float>(floor(vmapnamebackSize.y * fHeight/ fRealImageY));

	D3DXVECTOR2 vmapnamebackAlign;
	vmapnamebackAlign.x = (fWidth - vmapnamebackAlignSize.x)/2.0f;
	vmapnamebackAlign.y = static_cast<float>(floor(vmapnamebackTexPos.y * fHeight/ fRealImageY));

	D3DXVECTOR2 vhinticonAlign;
	vhinticonAlign.x = /*vhinticonSize.x+*/25.0f;
	...
	{
		TEXTUREVERTEX VertexArray[6];
		SetVertexPos ( VertexArray, ROOT_LEFT + vmapnamebackAlign.x, ROOT_TOP + vmapnamebackAlign.y, vmapnamebackAlignSize.x, vmapnamebackAlignSize.y  );
	...
	CD3DFontPar* pFont_12 = DxFontMan::GetInstance().LoadDxFont( _DEFAULT_FONT, 12, _DEFAULT_FONT_FLAG );
	if( pFont )
		pFont->UsageCS( TRUE );

	if( pFont_12 )
		pFont_12->UsageCS( TRUE );
	...
	while ( n_bRender )
	{
		...
		if(m_vectip.size ()==0)
		{
			CString strFullPath = n_szAppPath;
		...
		if( SUCCEEDED( pd3dDevice->BeginScene() ) )
		{
			...
			if( pFont )
			{
				...
				{
					pFont->DrawText(vhinticonAlign.x+85.0f,vhinticonAlign.y+15.0f,NS_UITEXTCOLOR::WHITE,m_vectip[nIndex1].GetString(), 0L, FALSE);	
				}
				
				if( nIndex2 > 0  && nIndex2 < m_vectip.size() )
				{
					pFont->DrawText(vhinticonAlign.x+85.0f,vhinticonAlign.y+35.0f,NS_UITEXTCOLOR::WHITE,m_vectip[nIndex2].GetString(), 0L, FALSE);
				}
			}
			...
			if( pFont_12 )
			{
				pFont_12->DrawText( vmapnamebackAlign.x + ( vmapnamebackSize.x / 3 ), vmapnamebackAlign.y + ( vmapnamebackSize.y / 4 ), NS_UITEXTCOLOR::WHITE,strTempMap.GetString(), 0L, FALSE);
			}
...
namespace	NLOADINGTHREAD
{
	...
	bool  LoadTip( const CString& strFileName )
	{
