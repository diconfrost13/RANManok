...
void CCrowTargetInfoPlayer::CreateSubControl ()
{
	...
	m_pTargetBox->CreateBaseBoxDialogueWhite ( "RNCROW_TARGET_INFOBOX_PLAYER" );
...
{	
		...
		m_pClassBox->SetOneLineText ( strClassCombine, NS_UITEXTCOLOR::WHITE );

		CString strRebornCombine;
		strRebornCombine.Format ("Reborn: %d", strReborn );
		m_pRebornBox->SetOneLineText ( strRebornCombine, NS_UITEXTCOLOR::WHITE );

		if(strGuild != ""){
		CString strGuildCombine;
		strGuildCombine.Format ("%s", strGuild);
		m_pGuildBox->SetOneLineText ( strGuildCombine, NS_UITEXTCOLOR::WHITE );
		}else{
		CString strGuildCombine;
		...
		m_pTextBox->SetOneLineText ( strPartyCombine, NS_UITEXTCOLOR::DODGERBLUE );
		}else{
		CString strPartyCombine;
