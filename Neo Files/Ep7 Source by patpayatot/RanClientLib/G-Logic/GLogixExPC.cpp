...
//			for ( i=0; i<SKILLFACT_SIZE; ++i )		DISABLESKEFF ( i );
	...
	if(m_fPkTimer >= 30.0f) m_dwPkCounter = 0;
}
...
void GLCHARLOGIC::STATSUP_CMD( EMSTATS emStats, WORD value ){

	switch ( emStats )
	{
	case EMPOW:
		{
			if ((m_sStats.wPow + value) > GLCONST_CHAR::wPowMax )
	...
	case EMSTR:
		{
			if ((m_sStats.wStr + value) > GLCONST_CHAR::wStrMax )
			{}else if( m_wStatsPoint >= value ){
				m_sStats.wStr += value;
				m_wStatsPoint -= value;	}
		}
	...
	case EMSPI: 
		{
			if ((m_sStats.wSpi + value) > GLCONST_CHAR::wSpiMax )
			{}else if( m_wStatsPoint >= value ){
				m_sStats.wSpi += value;
				m_wStatsPoint -= value;	}
		}
	...
	case EMDEX: 
		{
			if ((m_sStats.wDex + value) > GLCONST_CHAR::wDexMax )
			{}else if( m_wStatsPoint >= value ){
				m_sStats.wDex += value;
				m_wStatsPoint -= value;	}
		}
	...
	case EMINT:
		{
			if ((m_sStats.wInt + value) > GLCONST_CHAR::wIntMax )
			{}else if( m_wStatsPoint >= value ){
				m_sStats.wInt += value;
				m_wStatsPoint -= value;	}
		}
	...
	case EMSTA: 
		{
			if ((m_sStats.wSta + value) > GLCONST_CHAR::wStaMax )
			{}else if( m_wStatsPoint >= value ){
				m_sStats.wSta += value;
				m_wStatsPoint -= value;	}
		}
