...
{
	// StringCchCopyN(m_szServerName, DB_SVR_NAME_LENGTH, szName, DB_SVR_NAME_LENGTH);
	strcpy(m_szServerName, szName);
}

void CDbSupervisor::SetUserID(const char* szUsrID)
{
	// StringCchCopyN(m_szUsrID, USR_ID_LENGTH, szUsrID, USR_ID_LENGTH);
	strcpy(m_szUsrID, szUsrID);
}

void CDbSupervisor::SetUserPass(const char* szUsrPasswd)
{
	StringCchCopyN(m_szUsrPass, USR_PASS_LENGTH, szUsrPasswd, USR_PASS_LENGTH);
}
