...
CClientGame::CClientGame(int nMaxClient, COverlapped* pOverlapped, HANDLE	hIOServer) 
	: CClientManager(nMaxClient, pOverlapped, hIOServer)
{
}
...
int CClientGame::GetUserNum(DWORD dwClient) 
{
	int nClient;
	nClient = m_pClient[dwClient].GetUserNum();	
	return nClient;
}
