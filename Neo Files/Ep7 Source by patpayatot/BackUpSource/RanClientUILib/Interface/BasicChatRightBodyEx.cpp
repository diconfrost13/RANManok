...
bool CBasicChatRightBody::SEND_PRIVATE_MESSAGE( const CString& strOrigin )
{
	...
	CString strCombine;
	strCombine.Format ( "[Whisper to %s]:%s", strName, srtTemp.GetString () );    //쵱刀
	...
	case CHAT_TEXTBOX_ALL:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( UIMSG_LB_DUP & dwMsg )
				{
					...
					int nStringStart = strCharName.Find( _T('](') );
					if( nStringStart != - 1 )
					{
						INT nCnt = strCharName.Find( _T('):') );

						BEGIN_PRIVATE_CHAT( strCharName.Mid( nStringStart + 1, (nCnt - 1) - ( nStringStart - 1 ) - 2 ) );
					}
				}
			}
		}
...
void CBasicChatRightBody::AddStringToNORMAL ( CBasicTextBox::STEXTCHAT strTemp, D3DCOLOR dwColor )
{
	if ( m_pTextBox_ALL && m_pScrollBar_ALL )
	{
		m_pTextBox_ALL->AddTextChat ( strTemp, dwColor );
...
void CBasicChatRightBody::AddStringToNORMAL ( CString strTemp, D3DCOLOR dwColor, bool bBOOL )
{
	...
	AddStringToNORMAL ( sTemp, dwColor );
}
...
void CBasicChatRightBody::AddStringToChatEx ( CString strTemp, WORD wType )
{
	...
	case CHAT_PRIVATE:
		{
			...
			ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwTempColor, m_pTextBox_PRIVATE, m_pScrollBar_PRIVATE );
		}
		break;	
	...
	case CHAT_GUILD:
		{
			...
			ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwTempColor, m_pTextBox_CLUB, m_pScrollBar_CLUB );
		}
		break;

	case CHAT_TOALL:
		{
			sTemp.strHeader = "[Shouting]";
	...
	case CHAT_PARTY:
		{
			...
			ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwTempColor, m_pTextBox_PARTY, m_pScrollBar_PARTY );
		}
		break;

	case CHAT_ALLIANCE:
		{
			sTemp.strHeader = "[Alliance]";
			dwTempColor = NS_UITEXTCOLOR::HOTPINK;
			ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwTempColor, m_pTextBox_ALLIANCE, m_pScrollBar_ALLIANCE );
		}
		break;
	...
	case CHAT_SYSTEM:			//에러 또는 시스템 메시지
		{
			...
			ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwTempColor, m_pTextBox_SYSTEM, m_pScrollBar_SYSTEM );
		}
		break;
	}
	...
	AddStringToNORMAL ( sTemp, dwTempColor );
...
void CBasicChatRightBody::AddStringToSystemMessage ( CString strTemp, D3DCOLOR dwColor )
{
	...
	sTemp.strHeader = "[System]";
	sTemp.strMain = strTemp;
	ADD_CHAT_PAGE_TEXTBOX ( sTemp, dwColor, m_pTextBox_SYSTEM, m_pScrollBar_SYSTEM );
...
{	
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_NORMAL_MODE"), NS_UITEXTCOLOR::DEFAULT, true );	
...
{	
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_PRIVATE_MODE"), NS_UITEXTCOLOR::PRIVATE, true );
...
void	CBasicChatRightBody::BEGIN_PARTY_CHAT ( const CString& strMessage )
{
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_PARTY_MODE"), NS_UITEXTCOLOR::AQUAMARINE, true );
...
void	CBasicChatRightBody::BEGIN_TOALL_CHAT ( const CString& strMessage )
{
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_TOALL_MODE"), NS_UITEXTCOLOR::ORANGE, true );
...
void	CBasicChatRightBody::BEGIN_GUILD_CHAT ( const CString& strMessage )
{
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_GUILD_MODE"), NS_UITEXTCOLOR::VIOLET, true );
...
void	CBasicChatRightBody::BEGIN_ALLIANCE_CHAT( const CString& strMessage )
{
	...
	AddStringToNORMAL ( ID2GAMEINTEXT("CHAT_ALLIANCE_MODE"), NS_UITEXTCOLOR::HOTPINK, true );
